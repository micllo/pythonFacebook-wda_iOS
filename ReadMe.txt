...
（3）get_app_bundleId           通过项目名称 获取APP应用的'bundleId'
...
upstream api_server_Facebook_wda_iOS{
  server 127.0.0.1:4001 weight=1 max_fails=2 fail_timeout=30s;
  ip_hash;
}
...
server {
  ...
  location /test_report_local/ {
        ...
        alias /Users/micllo/Documents/works/GitHub/pythonFacebook-wda_iOS/Reports/;
       }
  ...
  location /api_local/ {
         ...
         proxy_pass http://api_server_Facebook_wda_iOS/;
...
3.访问地址（ server.py 启动 ）：
（1）接口地址 -> http://127.0.0.1:4002/
               http://127.0.0.1:4002/API/index
               http://127.0.0.1:4002/API/get_project_case_list/<pro_name>

4.访问地址（ uwsgi 启动 ）：
（1）用例页面 -> http://localhost:4010/api_local/iOS/index
（2）测试报告 -> http://127.0.0.1:4010/test_report_local/<pro_name>/[iOS_report]<pro_name>.html
（3）接口地址 -> http://127.0.0.1:4010/api_local/
               http://127.0.0.1:4010/api_local/iOS/sync_run_case
               http://127.0.0.1:4010/api_local/iOS/get_img/5e5cac9188121299450740b3
   （ 备注：uwgsi 启动 4001 端口、nginx 配置 4010 反向代理 4001 ）
